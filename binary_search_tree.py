...
class BST(object):
    ...
    def _bst_remove(self, subtree, key):
        if subtree is None:
            ...
            if subtree.left is None and subtree.right is None:  # 要删除的节点是叶子结点
                return None
            elif subtree.left is None or subtree.right is None:  # 要删除的节点有一个孩子
                if subtree.left is not None:  # 返回它的孩子并且让它的父亲指过去
                    return subtree.left
                else:
                    return subtree.right
            else:  # 要删除的节点右两个孩子，寻找后继结点并且替换
                successor_node = self._bst_min_node(subtree.right)
                subtree.key, subtree.value = successor_node.key, successor_node.value
                subtree.right = self._bst_remove(subtree.right, successor_node)
                return subtree
